...
| 进度页 | 进度概览 | 五大阶段的进度环、已掌握知识点数量 |
| 进度页 | 成就徽章 | 解锁式徽章展示（首次运行代码、完成 10 题等） |
| 进度页 | 学习日历 | 简易热力图，记录每日学习情况 |
| 进度页 | 重置进度 | 一键清空 localStorage 中的所有进度数据 |

### 2.4 练习题型

随堂练习（`Exercise`）支持三种题型，覆盖从概念辨析到代码执行预测的不同认知层级：

| 题型 | `type` 值 | 适用场景 | 交互 |
|------|-----------|----------|------|
| 选择题 | `multiple-choice` | 概念辨析、关键字识别 | 单选选项列表，提交后比对 `answer` |
| 填空题 | `fill-blank` | 关键函数 / 语法背诵 | 文本输入框，提交后按字符串比对 |
| 预测输出 | `predict-output` | 巩固代码执行结果 | 给定代码片段，要求写出输出 |

每道题都附带 `hint`（提示）与 `explanation`（解析），提交错误时可引导学习。

### 2.5 成就徽章系统

徽章用于增强学习反馈与持续动力，分为**章节完成类**、**代码运行类**、**练习答题类**三类，共 7 个：

| ID | 名称 | 分类 | 解锁条件 |
|----|------|------|----------|
| `first-lesson` | 初出茅庐 | 章节 | 完成第 1 个章节 |
| `five-lessons` | 小有所成 | 章节 | 完成 5 个章节 |
| `all-rounder` | 通关玩家 | 章节 | 完成 15 个章节 |
| `first-run` | Hello World | 代码 | 首次运行代码 |
| `runner` | 代码狂人 | 代码 | 累计运行代码 10 次 |
| `quiz-novice` | 答题新手 | 练习 | 答对 5 道练习 |
| `quiz-master` | 答题达人 | 练习 | 答对 15 道练习 |

徽章定义在 `src/pages/Progress.tsx` 的 `BADGES` 数组，解锁逻辑在 `src/store/useProgressStore.ts` 的状态变更函数中触发。
...
每个阶段（`Stage`）在数据层拥有以下元信息，用于驱动 UI 渲染与颜色主题：

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | `StageId` | 阶段唯一标识，如 `basics`、`oop` |
| `index` | `number` | 阶段序号（1-5），用于 `STAGE 0X` 角标 |
| `title` | `string` | 阶段中文名，如「Python 入门基础」 |
| `tagline` | `string` | 一句话副标题，展示在卡片顶部 |
| `description` | `string` | 阶段详细介绍 |
| `accent` | `'vine' \| 'ember' \| 'sky' \| 'rose' \| 'violet'` | 阶段主色，决定进度环颜色 |
| `icon` | `'spark' \| 'stack' \| 'cube' \| 'library' \| 'rocket'` | 阶段图标，映射到 lucide-react |
| `lessonIds` | `string[]` | 阶段下包含的章节 id 列表 |

## 6. 非目标 / 后续规划

为保持本期交付聚焦，以下能力**本期不实现**，留待后续迭代：
...
- **完成率**：进入章节的用户中，完成全部练习的占比
- **运行次数**：人均代码运行次数（衡量动手程度）
- **连续打卡**：7 日 / 30 日留存（基于 `streakDays` 与 `studyLog`）
- **徽章分布**：7 个徽章的解锁分布，识别难点章节
